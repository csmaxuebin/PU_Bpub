...
for col in col_all:
    ...
    if ((not single_err)):
        ...
        leng=leng+1
        t1=SVM_ratio(train_x, train_y, test_x, test_y, loop_time,'RF')
        m1=m1+t1
        if (not single_err2):
            t2=0.5
        else:
            t2=SVM_ratio(train_x2, train_y2, test_x, test_y, loop_time,'RF')
            # m2+=t2
        m2=m2+t2
        print('col:',col,' ',t1,t2)
...
with open('file-' + str(390) + '1-RF_syn_datatime_R.csv', 'a') as fid:
